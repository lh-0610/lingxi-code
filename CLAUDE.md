...
- **同一项检查重试通过，旧失败要让位**（`effective_rows` / `check_identity`）：身份用**采集时算的指纹** `verification.identity_fingerprint`（检查器 + 目录 + 目标 + **完整未截断**的 argv/命令的 sha256），同指纹后来的取代先前的，旧记录保留展示并标 `superseded`。不这么做的话，文件没改所以旧失败**不会过期**，`all()` 一直把它算进去，卡片停在"检查未全部通过"，而完成闸门早就认了 `tests_passed=True`——两边对同一件事给出相反结论。identity 必须带目录和参数：对两个文件跑 ruff、在主仓库和隔离区各跑一次 pytest，都是两项检查，不该互相顶替
- **指纹必须在截断之前算**：展示字段是要脱敏截断的（argv 每项 300 字、命令 400 字），拿它们比身份会把长参数的差异整个抹掉。实测两组 `pytest -k <702 字符>` 只有末尾不同，截断后完全一样，于是第一组的失败被第二组"取代"、卡片写成"已执行检查通过"，而第二组根本没跑那个失败用例。指纹是单向哈希，完整命令带密钥也不会因此泄露
- 没有指纹的旧记录（本批之前存下的）**一律各算各的、永不互相取代**：宁可停在"检查未全部通过"，也不能靠截断数据蒙一个"通过"出来
...
- **sidecar 里解析不了的条目不丢**（`_read_inflight_raw` 放进 `malformed`，改写时原样带回，`classify_inflight` 报 `unreadable`）：早先静默过滤——唯一那条记录坏了就等于"没有未决操作"，下一次改写还会把它彻底抹掉。恢复按"读不懂"处理（开盲区、有副作用的悬空调用按可能已执行说明），交接后才摘除，否则每一轮都会为它重开一次盲区
- **展示可以截断，留底必须完整**（`run_records.archive_malformed`）：摘除前把这些条目的**完整原文**写进 `chat_memory/<id>.inflight.unreadable-<内容哈希>.json`，恢复说明里只放摘要（`unreadable_count` + 最多 20 条 300 字预览）和对这个文件的引用（`unreadable_archive`）；只有留底成功、恢复说明落盘之后才摘，且只摘写进留底的那几条（`drop_malformed(..., archived)`）。早先只存截断预览就删全部原文——21 条损坏记录只剩 20 条截断预览，末尾内容和第 21 条永久丢失（第二轮复核实测）。文件名取内容哈希，同一批交接两次不会越攒越多；`delete_session` 经 `discard_inflight` 一并删除
- **work_root 按工具自己的目录规则记**（`tools_common.SHELL_CWD_TOOLS` / `tool_work_root`）：`run_command` / `run_tests` / `check_code` 在 `_shell_cwd()`（跟随 cd）里执行，其余按项目根解析路径。早先一律记 `_project_cwd()`——`cd B` 之后命令写在 B，记录却是项目 A（第二轮复核实测）。改了某个工具用哪个目录要同步这份清单，`test_b04_second_review.py` 有一条用例按源码核对：tools.py 里调用 `_shell_cwd()` 的实现恰好就是清单里的工具
...
- **放行按位置核对**（B04 第二轮复核）：义务按目录 / 位置记着，放行却只看全局的 `tests_passed` / `diff_reviewed` 的话，A 目录的改动能被 B 目录的一次检查清掉——实测：命令在 A 改了文件后中断，在 B 重开、只在 B 跑测试看 diff，真实 agent_loop 返回 completed。现在 `mark_tests(..., root=)` / `mark_diff_reviewed(root=)` 记下检查**实际**在哪儿做的（`tests_roots` / `diff_roots`；run_tests 报它的 `run_cwd`，git_diff 报执行目录、限定 path 时报那个路径），`mark_dirty(..., abs_path=)` 记下改动文件的实际位置（`dirty_abs`，经 `pending_verification.file_paths` 跨轮持久化）。`gaps_from_state` 要求：盲区根目录与位置已知的代码改动被某个测试目录覆盖、盲区与位置已知的改动被某个 diff 范围覆盖；不覆盖就保持未验证，并点名去哪儿补查。
- **"覆盖"= 路径包含 + 同一个 Git 工作区**（`verification.covers`，B04 第三轮复核）：只看路径包含是不够的——灵犀的隔离区就放在主项目的 `.lingxi-worktrees/` 下面，路径上"被包含"，却是另一份文件现场，主项目的测试跑不到它、`git diff` 也看不见它的改动（实测：隔离区里改了 app.py 后中断，重开回到主项目、只查主项目，返回 completed）。工作区的根 = 从路径往上第一个带 `.git` 的目录，`.git` 是目录（普通仓库、独立的嵌套仓库）或文件（linked worktree、子模块）都算边界；共享 Git 历史也不算同一个现场。两边都不在工作区里时退回只按路径包含。路径不存在（被删的文件）就从仍存在的上级找起。单次核对内用局部缓存复用"路径属于哪个工作区"（逐级 stat 在 Windows 上不便宜），不跨调用保留失效规则与两个全局标志完全相同。**没带 root 的检查不覆盖任何按位置记着的义务**——合成证据的测试也要写明出处
- 两类盲区不参与按位置核对，仍由全局检查了结，否则就是死结：占位键 `（工作目录无法确定）`（不是路径，无从谈覆盖）、**已不存在的目录**（没法再在那里跑检查）。搬家不走这里：义务已迁到新位置，照常按新目录核对
- **第一版的边界**：`git_diff` 固定在项目目录执行（不跟随 cd），所以位于当前项目之外、或属于另一个 Git 工作区（隔离区 / 子模块 / 嵌套仓库）的义务，在本会话里只能 cd 过去补跑测试、不能看 diff，按设计保持未验证，提示把那个目录作为项目打开再查看。项目里改了子模块文件的任务因此收不了 completed，这是如实反映"主项目的 diff 看不见子模块里的改动"。另有一处**早于 B04 的**缺口：`run_command` 执行中的工作区追踪只看项目目录，`cd` 到项目外之后命令的写入，进程不崩的情况下也看不见（崩了才会经执行前记录的 `work_root` 补上义务）
- **`git_diff` 的"算不算审阅过"判据是「这次调用有没有覆盖到该看的范围」，不是「有没有输出」**（`tools_git.py`）：带 `path` 限定的空 diff 只说明那一个文件没动，说明不了别处，不算；`staged=True` 的空 diff 只说明没 add 过，不算；非空输出照常算。执行失败（git 缺失 / 非仓库 / 非零退出 / 超时 / 路径逃逸）一律不算
- **全工作区的空 diff 还要再核对一次才算**（`_uncovered_changes`）：默认 `git diff` **只看已跟踪文件的未暂存改动**——`git add` 过的修改和未跟踪的新文件它一个都看不见。所以"输出为空"不等于"整个项目干净"。实测过：app.py 改完 add 了、以及新增一个未跟踪的 new_module.py，两种情况下这条命令都回"工作区干净"，义务被清空、任务按 completed 收尾，而那些改动从头到尾没在任何 diff 里露过面。现在解除**全局**义务前会再跑 `git diff --cached --name-only` + `git ls-files --others --exclude-standard`，有没覆盖到的就不解除，并在返回里点名文件、告诉模型去哪儿补看（`git_diff(staged=True)` / `git_status`）。`--exclude-standard` 尊重 `.gitignore`，构建产物不会把义务卡死
- 核对本身失败（非零 / 超时）→ **保留义务**并说明原因。核对不了就是不知道，不知道不放行（与追踪枚举失败同一取舍）
